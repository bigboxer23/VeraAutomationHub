...
@Tag(name = "Token Distribution", description = "Service with endpoints to help with distributing tokens to unauthenticated devices")
public class TokenDistributionServlet
{
	 ...
	 */
	@GetMapping(value = "/enableTokenFetch",
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Operation(summary = "Allows `getToken` endpoint to return the token.",
			description = "Enables an endpoint to distribute the token, but this endpoint itself is protected by" +
					" the normal authentication.  Essentially allows a device/user that already is trusted to grant" +
					" access to another unauthenticated device/user")
	...
	@GetMapping(value = "/getToken",
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Operation(summary = "Set the token in a cookie and return it",
			description = "Sets the token in a cookie and returns a JSON object with the token.  This servlet will return" +
					" unauthorized unless the `enableTokenFetch` endpoint is first called.  This endpoint is not protected " +
					"by needing the token passed with it.")
	@ApiResponses({@ApiResponse(responseCode = HttpURLConnection.HTTP_UNAUTHORIZED + "", description = "unauthorized, if `enableTokenFetch`" +
			" isn't called first"),
			@ApiResponse(responseCode = HttpURLConnection.HTTP_OK + "", description = "success")})
